...
impl InterfacePool {
    ...
    fn lib_id(&mut self, name: &str) -> FResult<usize> {
        ...
        if let Some(lpath) = self.libpaths.get(name) {
            let lib = unsafe { Library::new(lpath) }.map_err(|e| {
                new_error(
                    ErrorType::NativeLoadFailure,
                    format!("Failed to load native lib \'{name}\' [from {lpath}], cause: {e:?}"),
                )
            })?;
            ...
            // Don't allow random libraries to be loaded / initialized.
            Err(new_error(
                ErrorType::UnknownLibrary,
                format!("Cannot load unknown shared library \'{name}\'."),
            ))
        }
    }
    ...
    fn handle_by_index(&self, lib_id: usize, f_id: usize) -> FResult<NativeHandle> {
        self.ifaces
            .get(lib_id)
            .and_then(|a| a.functions.get(f_id))
            .copied()
            .ok_or_else(|| {
                new_error(
                    ErrorType::NoSuchFunction,
                    format!("Failed to retrieve native handle #{f_id} from lib{lib_id}"),
                )
            })
    }
    ...
    fn load_handle(&mut self, libname: &str, fname: &str) -> FResult<(usize, usize, NativeHandle)> {
        ...
        if let Some(&index) = niface.funcname_map.get(fname) {
            // If function is already loaded, just return it.
        ...
        let handle = unsafe {
            self.libs
                ...
                .unwrap()
                .get(fname.as_bytes())
                .map(|s: Symbol<NativeHandle>| *s)
        }
        .map_err(|e| {
            new_error(
                ErrorType::NoSuchSymbol,
                format!("Could not obtain symbol \'{fname}\' from \'{libname}\', cause: {e:?}"),
            )
        })?;

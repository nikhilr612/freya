...
where F: (FnOnce(T) -> E) + Send + 'static + Copy,
	T: Send + 'static,
	E: Send + 'static {
	...
	let n_jobs = list.len();
	let n_workers = n_jobs.min(available_parallelism().expect("Failed to obtain available parallel compute / parallelism.").into());
    ...
    for elm in list {
        let ltx = tx.clone();
        pool.execute(move || {
            ltx.send(op(elm)).expect("Failed to send result message to receiver channel.")
        })
    }
    ...
    info!("Completed {n_succ}/{n_jobs} jobs ({:.3}%).", (n_succ as f64) / (n_jobs as f64) * 100.0);

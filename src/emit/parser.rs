...
use std::io::Read;
use crate::emit::{TextualLocation, Sexpr};
...
fn int_from_str_radix_auto(s: &str, loc: &TextualLocation) -> Result<i64, TlError> {
  let radix = match s.chars().nth(1) {
  	...
  	Some(c) => {
  		return Err(TlError { etype: TlErrorType::UnexpectedToken, msg: format!("Invalid radix prefix '0{c}'. Expected '0x', '0b', or '0o'."), loc: *loc})
  	}
  };
  i64::from_str_radix(&s[2..], radix)
  	.map_err(|e| {
  		let msg = format!("'{s}' is not an integer in base {radix},\n\tdetail: {e}");
  		TlError { etype: TlErrorType::InvalidNumeric, msg,  loc: *loc }
  	})
}
...
where T: Read {
	...
	if s.len() < 4 {
		Err(TlError { etype: TlErrorType::UnexpectedEOF, msg: format!("Unicode escape requires exactly 4 hex digits, but only '{s}' was provided."), loc })
	} else {
		u32::from_str_radix(&s, 16)
			...
			.and_then(|v| {
				char::from_u32(v)
				.ok_or_else(||
					TlError { etype: TlErrorType::InvalidChar, msg: format!("Hex '{s}' does not signify a character."), loc 
				})
			})
	}
}
...
where T: Read {
	...
	if expect_char(stream)? != '\'' {
		Err(TlError { etype: TlErrorType::InvalidChar, msg: format!("Character literal '{ch}' must be enclosed by single quotes."), loc })
	} else {

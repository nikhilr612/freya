...
use crate::{core::{module::{ExternDecl, FuncDecl}, op::{self, DoubleRegst, Id16Reg, QuadrupleRegst, VariadicRegst}}, utils::{AsBytes, OutBuf}};
...
impl CompileUnit {
	...
	fn finish(self) -> Result<(), std::io::Error> {
		todo!("Write header and copy out contents from temporary file. An important step here is to fix all incorrect offsets generated by walking.")
	}
}
...
fn walk_define_form(v: Vec<Sexpr>, cu: &mut CompileUnit, sc: &mut Scope<'_>) -> Result<(), TlError> {
	...
	let fidx = if let SymbolType::InternalFunction { index } = sc.resolve(&name).expect("Function name should already be bound.") {
    	cu.functions[index as usize].offset = cu.offset();
    	index as usize
    } else {
...
fn check_regs_adjacent(v: &Vec<u8>) -> bool {

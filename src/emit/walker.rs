...
use std::path::Path;
use log::debug;
use std::{collections::HashMap, fs::{File, OpenOptions}, io::{Seek, Write}};
use crate::{core::{module::{ExternDecl, FuncDecl, MAGIC}, op::{self, DoubleRegst, Id16Reg, QuadrupleRegst, VariadicRegst}}, utils::{AsBytes, OutBuf}};
...
impl CompileUnit {
	...
	fn emit_simple(&mut self, opcode: u8, v: u8) -> Result<(), TlError> {
		self.tmpfile.write_u8(opcode).map_err(CompileUnit::_write_error)?;
	...
	fn emit(&mut self, opcode: u8, content: impl AsBytes) -> Result<(), TlError> {
		self.tmpfile.write_u8(opcode).map_err(CompileUnit::_write_error)?;
		content.write(&mut self.tmpfile).map_err(CompileUnit::_write_error)
	}

	fn emit_lineno(&mut self, sx: &Sexpr) -> Result<(), TlError> {
		self.tmpfile.write_u8(op::DBGLN).map_err(CompileUnit::_write_error)?;
		self.tmpfile.write_u16(sx.loc.line_no as u16).map_err(CompileUnit::_write_error)
	}

	/// Get the stream position of the underlying file.
	fn offset(&mut self) -> u32 {
		self.tmpfile.stream_position().expect("CompileUnit temporary file should have stream_position") as u32
	}

	fn _write_error(e: std::io::Error) -> TlError {
		TlError {
			etype: TlErrorType::IoError, msg: format!("Failed to write to temporary file backing compile unit,\n\tdetail: {e}"),
			loc: TextualLocation {
				start_offset: 0,
				end_offset: 0,
				line_no: 0 
	...
	where F: AsRef<Path> {
		...
		let header_end = (file.stream_position()? as u32) + (self.functions.iter().fold(0, |acc, x| {acc + 7 + x.0.len()}) as u32);
...
fn visit_define_form<'a>(mut it: impl Iterator<Item = &'a Sexpr>, cu: &mut CompileUnit, sc: &mut Scope<'_>, loc: &TextualLocation) -> Result<(), TlError> {
	...
	match &a.kind {
		SexprKind::Atom(Token::Symbol(s)) => {
			...
			let sym = match tok {
			    Token::Integer(v) if (-0x80..=0x7f).contains(&v) => SymbolType::NamedInteger { value: v as i8 },
			    Token::Boolean(v) => SymbolType::NamedInteger { value: v as i8 },
			    Token::Symbol(_) => panic!("Somehow the token which was already checked for is now a symbol."),
			    tok => {
			    	let new_id = cu.constants.len();
					let new_id = *cu.constants.entry(tok).or_insert(new_id);
					SymbolType::NamedConstant { pool_index: new_id as u16 }
			    }
			};
			sc.bind(s, sym)
			.map_err(|_| TlError {
...
fn eat_literal_atom(t: Token, cu: &mut CompileUnit, r1: u8) -> Result<(),TlError> {
	match t {
		Token::Integer(i) if i > -0x80 && i < 0x7f => {
			return cu.emit(op::LDI, DoubleRegst {r1: (i as i8) as u8, r2: r1});
		},
		Token::Boolean(i) => {
			return cu.emit(op::LDI, DoubleRegst {r1: (i as i8) as u8, r2: r1});
		}
...
fn walk_define_form(v: Vec<Sexpr>, cu: &mut CompileUnit, sc: &mut Scope<'_>) -> Result<(), TlError> {
	let mut v = v.into_iter().skip(1);
	let b = v.next().expect("Validated `define` form should have head Sexpr");
	let c = v.next()
	...
	let fparams = b.collect::<Result<Vec<_>, _>>()
		.map_err(|e| e.aug("Looking for (func-name param..) symbol list as function-type `define` head."))?;

	let fidx = if let SymbolType::InternalFunction { index } = sc.resolve(&name).expect("Function name should already be bound.") {
	...
	match c.kind {
	    ...
	    SexprKind::Atom(Token::Symbol(s)) => {
	    	if s == "None" {
	    		cu.tmpfile.write_u8(op::VRET).map_err(CompileUnit::_write_error)
	    		...
	    		cu.emit(op::LDC, Id16Reg { id: pool_index, r1: 0 })?;
	    		cu.emit_simple(op::RET, 0)
	    	} else {
	    		let mut it = fparams.iter().enumerate().filter(|&(_, p)| p == &s).map(|(a, _)| a).take(2);
	    ...
	    SexprKind::List(v) => {
	    	...
	    	for (register, name) in ralloc.many(fparams.len()).zip(fparams.into_iter()) {
	    		...
	    		function_scope.bind(name, SymbolType::NamedValue { register }).map_err(|_| TlError {
			        etype: TlErrorType::ReboundName,
			        msg: "Duplicate binding names are not allowed for function formal parameters".to_owned(),
			        loc: c.loc,
			    })?;
	    	}
...
fn walk_regular_list(
	...
	cu: &mut CompileUnit, sc: &mut Scope<'_>, 
	loc: TextualLocation, 
	cleanup: bool
) -> Result<u8, TlError> {
	...
	cu.emit_lineno(&s0).map_err(|e| e.aug("Trying to emit line number debug information"))?;
	...
	if v.is_empty() {
		...
		cu.emit(op::FSTCALL, QuadrupleRegst { r1: callable_r, r2: (eval_reg + 1), s1, s2})?;
	} else {
		cu.emit(op::STDCALL, DoubleRegst { r1: callable_r, r2: (eval_reg + 1) })?;
		let v: VariadicRegst = v.into();
		v.write(&mut cu.tmpfile).map_err(CompileUnit::_write_error)?;
	}
	...
	if cleanup {
		...
		cu.emit(op::DEL, DoubleRegst { r1: eval_reg+1, r2: eval_reg + nt })?;
	}
	...
	if s == "None" {
		...
		match sc.resolve(&s) {
	        ...
	        Some(SymbolType::NamedInteger { value }) => {
	        	let r2 = ralloc.get();
	        	cu.emit(op::LDI, DoubleRegst {r1: value as u8, r2})?;
	        	Ok((r2, true))
	        },
...
fn walk_root_list(ast: Sexpr, cu: &mut CompileUnit, sc: &mut Scope<'_>) -> Result<(), TlError> {
	...
	let v = ast.list().expect("Visit pass should have error-ed out on illegal root-level lists.");
	let s = v
			.first()
	...
	if s != "define" { return Ok(()); } // Other forms (`require`) should be handled in visit phase itself.

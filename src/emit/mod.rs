...
impl core::fmt::Debug for TextualLocation {
    fn fmt(&self, fmt: &mut std::fmt::Formatter<'_>) -> std::result::Result<(), std::fmt::Error> {
        write!(
            fmt,
            "(loc: {}..{} ; {})",
            self.start_offset, self.end_offset, self.line_no
        )
    }
}
        ...
        match self {
            ...
            Self::Str(s) => {
                if s.len() >= MAX_STRING_DISPLAY_LEN {
                    write!(fmt, "Str(\"")?;
                    for ch in s.chars().take(MAX_STRING_DISPLAY_LEN) {
                        write!(fmt, "{ch}")?;
                    }
...
impl TlError {
    ...
    fn capture(
        itr: impl Iterator<Item = Result<(), TlError>>,
        ...
        F: FnMut(T) -> E,
    {
        ...
        if no_error {
            ...
            Err(TlError {
                etype: TlErrorType::Complex(errs),
                msg: msg.into(),
                loc,
            })
        }
...
impl std::fmt::Debug for TlError {
    fn fmt(&self, fmt: &mut std::fmt::Formatter<'_>) -> std::result::Result<(), std::fmt::Error> {
        match &self.etype {
            TlErrorType::IoError => write!(
                fmt,
                "TlError [{:?}], {:?}\n\tdetail: {}",
                self.etype, self.loc, self.msg
            ),

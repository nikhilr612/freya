...
use crate::core::types::{self, new_error, ErrorType, FatalErr};
...
impl RefCounter {
    ...
    pub fn incref_mut(&mut self, ptr: *mut types::CompositeType) -> FResult<()> {
        ...
        if let Entry::Vacant(e) = self.refctr.entry(addr) {
            ...
            if tup.1 > 0 {
                return self.error_or_warn(new_error(
                    ErrorType::MutableAliasing,
    ...
    pub fn decref(&mut self, ptr: *const types::CompositeType) -> FResult<()> {
        ...
        if self.refctr.contains_key(&addr) {
            ...
            if tup.0 > 0 {
                ...
                return self.error_or_warn(new_error(
                    ErrorType::NoRefsToDec,
                    format!("@{addr:x} has no counted immutable references."),
                ));
            }
        }
        self.error_or_warn(new_error(
            ErrorType::NoRefsToDec,
            format!("@{addr:x} has no counted immutable references."),
        ))
    }
    ...
    pub fn decref_mut(&mut self, ptr: *mut types::CompositeType) -> FResult<()> {
        ...
        if self.refctr.contains_key(&addr) {
            ...
            if tup.1 > 0 {
                ...
                return self.error_or_warn(types::new_error(
                    ErrorType::NoRefsToDec,
                    format!("@{addr:x} has no counted mutable references."),
                ));
            }
        }
        self.error_or_warn(types::new_error(
            ErrorType::NoRefsToDec,
            format!("@{addr:x} has no counted mutable references."),
        ))
    }

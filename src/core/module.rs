 ...
 impl ModulePool {
 	...
 	pub fn id_by_name(&self, path: &str) -> FResult<usize> {
 		...
 		let idx = match p.get(path) {
 			Some(i) => *i,
 			None => {
 				return Err(types::new_error(ErrorType::NoSuchModule, format!("Module path: {path} has not been loaded into the pool.")));
 			}
 		};
 		Ok(idx)
 	}
		...
		match view.get_u8() {
			...
			3 => {
				...
				let val = match char::from_u32(rval) {
					Some(ch) => ch,
					None => {
						return Err(types::new_error(ErrorType::UtfDecodeError, format!("Value {:x} is not a valid Unicode scalar", rval)));
					}
				};
			...
			_ => {
				return Err(types::new_error(ErrorType::ModuleLoadFailure, "Invalid type id for constant pool"));
			}
		}
	}

use crate::core::{new_error, ErrorType, FatalErr};
...
impl SliceView<'_> {
    ...
    pub fn decode_utf8(&mut self, len_bytes: usize) -> Result<String, FatalErr> {
        ...
        match retstr {
            ...
            Err(e) => Err(new_error(
                ...
                format!("Failed to decode slice {sl:?}\ncause:\t{e:?}"),
            )),
        }
    }
    ...
    pub fn get_u64(&mut self) -> u64 {
        let val = (self.buf[self.idx] as u64) << 56
            | (self.buf[self.idx + 1] as u64) << 48
            | (self.buf[self.idx + 2] as u64) << 40
            | (self.buf[self.idx + 3] as u64) << 32
            | (self.buf[self.idx + 4] as u64) << 24
            | (self.buf[self.idx + 5] as u64) << 16
            | (self.buf[self.idx + 6] as u64) << 8
            | (self.buf[self.idx + 7] as u64);
        self.idx += 8;
        val
    }

    pub fn get_u32(&mut self) -> u32 {
        let val = (self.buf[self.idx] as u32) << 24
            | (self.buf[self.idx + 1] as u32) << 16
            | (self.buf[self.idx + 2] as u32) << 8
            | (self.buf[self.idx + 3] as u32);
        self.idx += 4;
        val
    }
        ...
        let (s, v) = match std::str::from_utf8(&self.buffer[..blen]) {
            ...
            Err(e) => {
                if e.valid_up_to() == 0 {
                    ...
                    (
                        unsafe { std::str::from_utf8_unchecked(&self.buffer[..e.valid_up_to()]) },
                        e.valid_up_to(),
                    )
                }
            }

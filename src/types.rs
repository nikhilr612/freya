use std::ops::{BitOr, BitAnd, Add, Sub, Mul, Rem, Shl, Shr, BitXor};
use std::iter::zip;
	...
	pub(crate) fn pop(&mut self, refc: &mut RefCounter) -> Result<BaseType, FatalErr> {
		match self {
			CompositeType::Str(s) => {
				let ch = s.pop();
				match ch {
					None => Err(new_error(ErrorType::IllegalState, "Cannot 'pop' from empty string.")),
	...
	($defined_name: ident, $function: ident, $ch: literal) => {
		...
		pub fn $defined_name(rv1: &BaseType, rv2: &BaseType) -> Result<BaseType, FatalErr> {
			match (rv1, rv2) {
				...
				_ => {return Err(new_error(ErrorType::TypeMismatch, format!("Unsupported {} between {rv1}, {rv2}", $ch)));}
			}
		}	
	};
}
